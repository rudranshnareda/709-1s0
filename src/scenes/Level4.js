...
export default class Level4 extends Phaser.Scene {
  constructor() {
    super({ key: 'Level4' });
    this.LEVEL_NUMBER = 4;
    this.LEVEL_NAME   = 'Jal Mahal';
  }

  preload() {
    this.load.image('bg_level4',      'assets/backgrounds/level_4_jal_mahal.png');
    ...
    this.load.image('monster_mariposa', 'assets/collectibles/monsters/monster_mariposa.png');
    this.load.image('flower_tulip',   'assets/collectibles/flowers/flower_tulip.png');
    this.load.image('gem_memory',     'assets/collectibles/gem_memory.png');
  }

  create(data) {
    this.cameras.main.fadeIn(400, 0, 0, 0);
    this.add.image(0, 0, 'bg_level4').setOrigin(0, 0).setDisplaySize(1280, 720);
    ...
    this.platforms.create(150,  580, 'tile_platform').setScale(2, 1).refreshBody();
    this.platforms.create(380,  520, 'tile_platform').setScale(2, 1).refreshBody();
    this.platforms.create(610,  460, 'tile_platform').setScale(2, 1).refreshBody();
    this.platforms.create(840,  400, 'tile_platform').setScale(2, 1).refreshBody();
    this.platforms.create(1070, 460, 'tile_platform').setScale(2, 1).refreshBody();
    this.platforms.create(1200, 520, 'tile_platform').setScale(2, 1).refreshBody();
    ...
    this.checkpoints.create(840, 700, 'level4_mid');
    this.checkpoints.setupPlayerOverlap(this.player.sprite);

    this.enemyManager = new EnemyManager(this);
    this.enemyManager.spawnEnemy(380,  490, 'police', { patrolStart: 280, patrolEnd: 580, health: 4 });
    this.enemyManager.spawnEnemy(840,  370, 'goon',   { patrolStart: 740, patrolEnd: 1040 });
    this.enemyManager.spawnEnemy(1100, 430, 'police', { patrolStart: 1000, patrolEnd: 1250, health: 4 });

    this.collectibles = new CollectibleManager(this);
    this.collectibles.spawnMonster(615, 430, 'mariposa');
    this.collectibles.spawnFlower(845, 370, 'tulip');
    this.collectibles.spawnMemoryGem(1205, 490, 'memory_04');
    ...
    this.events.on('playerDied', () => {
      ...
      this.scene.pause();
      this.scene.launch('DeathScene', { parentScene: this.scene.key, checkpointX: cp?.x ?? 100, checkpointY: cp?.y ?? 640 });
    });
  }
